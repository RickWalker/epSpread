class Tweet {
  ...
  Region findRegion(PVector t) {
    //map to the same space!
    PVector newT = new PVector(0,0);
    newT.x = map(t.x, 0, imgX, 1, 1799);
    newT.y = map(t.y, 0, imgY, 1,915);
  ...
  String toString() {
    ...
    return mDate + " " + mText + " " + tweetRegion.getName() +" at " + mLocation;
  }
}
