...
class HeatmapOverlay {
	...
	HeatmapOverlay(TwitterFiltering gp, TwitterFilteringComponent parent) {
		this.parent = parent;
		this.gp = gp;
		gridres = 200;
	...
	void draw() {
		...
		gp.noStroke();
		float boxsize_x = TwitterFiltering.imgX / (float) (gridres - 1);
		float boxsize_y = TwitterFiltering.imgY / (float) (gridres - 1);
		// println(max_intensity);
		gp.colorMode(PConstants.HSB, 50);
		for (int y = 0; y < gridres - 1; y++) {
			for (int x = 0; x < gridres - 1; x++) {

				// fill( map(overlay[x][y], 0, max_intensity, 0, 255), 0, 0,
				// map(overlay[x][y], 0, max_intensity, 0, 255));
				// noStroke();
				float v = overlay[y * gridres + x];
				if (v != 0.0) {
					// println("Drawing!");
					// stroke(map(v, 0, max_intensity, 37, 2), 50, 50, map(v, 0,
					// max_intensity, 2, 40));
					// noStroke();
					// old version
					// fill(map(v, 0, max_intensity, 37, 2), 50, 50, map(v, 0,
					// max_intensity, 2, 40));
					// rect(x*boxsize_x + parent.imgPos.x, y*boxsize_y +
					// parent.imgPos.y, imgX/gridres-1, imgY/gridres-1);
					// new version, attempts to shade
					gp.beginShape(PConstants.TRIANGLES);
					gp.fill(PApplet.map(v, 0, max_intensity, 37, 2), 50, 50,
							PApplet.map(v, 0, max_intensity, 2, 40));
					gp.vertex(x * boxsize_x + parent.imgPos.x, y * boxsize_y
							+ parent.imgPos.y);

					gp.fill(PApplet.map(overlay[y * gridres + x + 1], 0,
							max_intensity, 37, 2), 50, 50, PApplet.map(
							overlay[y * gridres + x + 1], 0, max_intensity, 2,
							40));
					gp.vertex((x + 1) * boxsize_x + parent.imgPos.x, y
							* boxsize_y + parent.imgPos.y);

					gp.fill(PApplet.map(overlay[(y + 1) * gridres + x], 0,
							max_intensity, 37, 2), 50, 50, PApplet.map(
							overlay[(y + 1) * gridres + x], 0, max_intensity,
							2, 40));
					gp.vertex(x * boxsize_x + parent.imgPos.x, (y + 1)
							* boxsize_y + parent.imgPos.y);

					gp.vertex(x * boxsize_x + parent.imgPos.x, (y + 1)
							* boxsize_y + parent.imgPos.y);

					gp.fill(PApplet.map(overlay[(y + 1) * gridres + x + 1], 0,
							max_intensity, 37, 2), 50, 50, PApplet.map(
							overlay[(y + 1) * gridres + x + 1], 0,
							max_intensity, 2, 40));
					gp.vertex((x + 1) * boxsize_x + parent.imgPos.x, (y + 1)
							* boxsize_y + parent.imgPos.y);
		...
		gp.colorMode(PConstants.RGB, 255);
	}
	...
	void createSimpleSurface(int w, int ht, ArrayList<Tweet> tweets) {
		...
		for (int i = 0; i < tweets.size(); i++) {
			if (parent.dateSelection.contains(tweets.get(i).mDate)) {
				 ...
				 * println("can't find region for " + tweets.get(i)); } //what
				 * time is it sent? boolean nighttime; if
				 * (tweets.get(i).mDate.getHourOfDay() <8 ||
				 * tweets.get(i).mDate.getHourOfDay() >6) { nighttime = true; }
				 * else { nighttime = false; }
				 * 
				 * //so, what weighting should it have? float weighting=0.0; if
				 * (r!=null) { if (nighttime) { weighting = 1.0 / (float)
				 * r.residents; } else { weighting = 1.0/(float)
				 * r.daytimePopulation; } }
				 * 
				 * //println(tweets.get(i).mLocation.x + " " +
				 * tweets.get(i).mLocation.y + " and " + xval+ " " + yval);
				 * overlay[yval*gridres + xval] += weighting;//1; //
				 * println(overlay[yval*gridres + xval]); max_intensity =
				 * max(overlay[yval*gridres + xval], max_intensity);
				 */
			}
		}
	}
	...
	void createWeightedSurface(int w, int ht, ArrayList<Tweet> tweets) {
		// clear old surface
		// gridres= 50;
		PApplet.println("Creating surface!");
		PApplet.println(parent.dateSelection);
		float boxsize_x = w;
		float boxsize_y = ht;
		...
		for (int i = 0; i < tweets.size(); i++) {
			...
			/*if (tweets.get(i).tweetRegion != null) { // do we have a region? if
				...
				if (tweets.get(i).mDate.getHourOfDay() < 8
						|| tweets.get(i).mDate.getHourOfDay() > 18) { // what
																		// time
																		// is it
																		// sent?
					nighttime = true;
				...
				if (nighttime) {
					masses[i] = (1.0f / (float) tweets.get(i).tweetRegion.residents); // resident
																						// weighting!
				} else {
					masses[i] = 1.0f / (float) tweets.get(i).tweetRegion.daytimePopulation; // daytime
																							// weighting
				}
				if (parent.dateSelection.contains(tweets.get(i).mDate)) {
					updateTweetCounts(tweets.get(i), daytimeTweets,
		...
		for (int i = 0; i < tweets.size(); i++) {
			...
			// so need to find the coordinates of the point twoh miles west of
			// it, then take xmin from that?
			if (parent.dateSelection.contains(tweets.get(i).mDate)) {
				// ipixmin = (int) ((cinemaLocations[i].lat - twoh - xmin) /
				// pixwidth_i);
				...
				// println(ipixmin + " " + ipixmax + " " + jpixmin + " " +
				// jpixmax);
				...
				for (ipix = ipixmin; ipix <= ipixmax; ipix++) {
					dx2i[ipix] = (((ipix - 0.5) * pixwidth - xpos[i]) * ((ipix - 0.5)
							* pixwidth - xpos[i]))
							* hi21; // + dz2;
				...
				// println("w_j is " + w_j);
				termnorm = 10. / (7. * PConstants.PI) * w_j;
				term = termnorm;
				...
				for (jpix = jpixmin; jpix <= jpixmax; jpix++) {
					...
					for (ipix = ipixmin; ipix <= ipixmax; ipix++) {
						...
						if (qq2 < 4.0) {
							...
							max_intensity = PApplet.max(overlay[jpix * gridres
									+ ipix], max_intensity);
						}
					}
				}
		...
		for (int i = 0; i < overlay.length; i++) {
			overlay[i] = PApplet.log(overlay[i] + 1.0f);
		}
		max_intensity = PApplet.log(max_intensity + 1.0f);

		// now that we have the grid, generate contours!
		// mangle array back into 2D
		// double [][] values = new double[gridres][gridres];
		/*
		 * for (int x = 0 ; x< gridres; x++) { for (int y = 0; y<gridres; y++) {
		 * //println(x +" " + y + " " + gridres); values[x][y]
		 * =overlay[y*gridres+x]; //println(values[x][y]); } }
		 */
		PApplet.println("Max intensity is " + max_intensity);
	}
	...
	void createSurface(int w, int ht, ArrayList<Tweet> tweets) {
		...
		// println("box size is " + boxsize_x + " " + boxsize_y);
		float[] masses = new float[tweets.size()];
		Arrays.fill(masses, 1.0f);
		Arrays.fill(overlay, 0.0f);
		xpos = new float[tweets.size()];
		ypos = new float[tweets.size()];
		for (int i = 0; i < tweets.size(); i++) {
			ypos[i] = tweets.get(i).mLocation.y; // distanceBetween(zerozero,
													// new
													// PVector(cinemaLocations[i].lat,
													// zerozero.lon));
			// if (tweets.get(i).mLocation.y > zerozero.y)
			// tweets.get(i).mLocation.y *= -1;

			xpos[i] = tweets.get(i).mLocation.x;// distanceBetween(zerozero, new
												// Location(zerozero.lat,
												// cinemaLocations[i].lon));
